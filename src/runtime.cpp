...
static XrResult XRAPI_PTR xrBeginSession_runtime(XrSession s, const XrSessionBeginInfo*) { 
    ...
    // A simulator has no physical HMD focus hand-off. Report input focus
    // immediately so action-state queries (teleport, grab, etc.) fire even
    // before the preview window exists; the activation handler may still move
    // between VISIBLE/FOCUSED later.
    rt::g_session.isFocused = true;
...
static XrResult XRAPI_PTR xrCreateReferenceSpace_runtime(XrSession, const XrReferenceSpaceCreateInfo* info, XrSpace* space) {
    ...
    *space = (XrSpace)(nextSpace++);
    ...
    // Also record the reference space for composition-layer pose resolution.
...
static XrResult XRAPI_PTR xrGetCurrentInteractionProfile_runtime(XrSession, XrPath topLevelUserPath, XrInteractionProfileState* interactionProfile) {
    ...
    if (++profileLogCount % 300 == 1) {
        Logf("[SimXR] xrGetCurrentInteractionProfile: topLevelUserPath=%llu -> profile=0x%llx",
             ...
             (unsigned long long)rt::g_activeProfile);
    }
    return XR_SUCCESS;
}
